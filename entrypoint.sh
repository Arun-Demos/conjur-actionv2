...
handle_git_jwt() {
    ...
    # Check if IAT less than or equal to server time
    if (( iat <= now )); then

        echo "::debug No delta between iat [$iat] and server time [$now]"

    # check if IAT greater than server time, if so, calculate delta and sleep before returning
    elif (( iat > now )); then

        delta=$(( iat - now ))
        echo "::debug delta found: iat [$iat] // server time [$now]; sleeping for $delta seconds"
        sleep "$delta"
